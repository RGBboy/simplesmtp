...
SMTPServer.prototype.end = function(callback){
    var i
    for (i = this.connectionPool.length - 1; i >= 0; i -= 1) {
      this._closeSMTPServerConnection(this.connectionPool[i]);
    }
...
SMTPServer.prototype._createSMTPServerConnection = function(client){
    this.connectionPool.push(new SMTPServerConnection(this, client));
};

/**
 * <p>Ends a {@link SMTPServerConnection} object and removes it from the connection pool.
 * 
 * @param {Object} connection SMTPServerConnection object.
 */
SMTPServer.prototype._closeSMTPServerConnection = function(connection) {
    if (this.connectionPool.indexOf(connection) !== -1) {
